 ...
 * carries its outcome.
 *
 * A resolved prop the user never forecast still lists its options — the
 * outcomes are the point of a resolved card, and hiding them behind "No
 * forecast yet" is exactly the case where the reader most wants to see what
 * happened. Only an unresolved prop with no forecast collapses to the
 * placeholder alone.
 *
...
export function ChoiceForecastSummary({
  ...
  const isResolved = options.some((option) => option.outcome !== null);
  if (!hasForecast && !isResolved) {
        ...
        {options.map((option) => {
          ...
          return (
            <div key={option.option_id} role="listitem" className="py-2">
              ...
              {/* The user's probability as a bar, so a column of rows can be
                  read at a glance the way the needle reads on a binary card.
                  Nothing to draw when the user never forecast. */}
              {hasForecast && (
