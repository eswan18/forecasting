...
const DateTimeInput = React.forwardRef<HTMLInputElement, DateTimeInputProps>(
  (options: DateTimeInputProps, ref) => {
    ...
    const curSegment = useMemo(() => {
      if (
        selectedSegmentAt === undefined || selectedSegmentAt < 0 ||
    ...
    const areAllSegmentsEmpty = useMemo(() =>
      validSegments.every((s) =>
        !s.value
      ), [validSegments]);

    const inputValue = useMemo(() => {
      const allHasValue = !validSegments.some((s) =>
        !s.value
      );
    ...
    const onClick = useEventCallback(
      (event: React.MouseEvent<HTMLInputElement>) => {
        ...
        if (
          ...
          !segment && (segment = [...validSegments].reverse().find((s) =>
            s.index <= selectionStart
          ));
          !segment && (segment = validSegments.find((s) =>
            s.index >= selectionStart
          ));
    ...
    const onSegmentValueRemove = useEventCallback(() => {
      ...
      if (curSegment.value) {
        ...
        setSegments(updatedSegments);
        const segment = updatedSegments.find((s) =>
