...
describe('Authentication - Login Integration Tests', () => {
  ...
  describe('login flow validation', () => {
    ...
    it('should handle missing user gracefully', async () => {
      ...
      const { login } = await import('./login');
      const result = await login({ username: 'nonexistent', password: 'password123' });
  ...
  describe('impersonation flow validation', () => {
    it('should require admin privileges for impersonation', async () => {
      ...
      await expect(loginViaImpersonation('targetuser')).rejects.toThrow('Not authorized.');
    });
    ...
    it('should validate target username exists', async () => {
      ...
      await expect(loginViaImpersonation('nonexistent')).rejects.toThrow('Invalid username.');
    });
  });
});