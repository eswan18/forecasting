...
export async function updateLoginPasswordFromResetToken({
  username,
  token,
  password,
}: {
  ...
  if (!user.login_id)
    throw new Error("Attempted password reset for user without login_id");
  // Validate the token.
  const tokenRecord = await db
    ...
    .executeTakeFirst();
  if (!tokenRecord)
    throw new Error("Attempted password reset with invalid token");
  if (tokenRecord.expires_at < new Date())
    throw new Error("Attempted password reset with expired token");
  // Update the password.
