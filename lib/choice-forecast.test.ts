...
describe("scoring", () => {
  it("binary is the plain Brier", () => {
    expect(scoreBinaryForecast(0.7, true)).toBeCloseTo(0.09, 10);
    expect(scoreBinaryForecast(0.7, false)).toBeCloseTo(0.49, 10);
  });
  it("one_of: uniform over 2/4/10 options scores 0.25 / 0.375 / 0.45", () => {
    ...
    expect(scoreChoiceForecast("one_of", uniform(2), oneHot(10, 2))).toBeCloseTo(0.25, 10);
    expect(scoreChoiceForecast("one_of", uniform(4), oneHot(10, 4))).toBeCloseTo(0.375, 10);
    expect(scoreChoiceForecast("one_of", u10, o10)).toBeCloseTo(0.45, 10);
  });
  it("one_of: a perfect forecast scores 0 and all mass on a loser scores 1", () => {
    ...
    expect(scoreChoiceForecast("one_of", perfect, oneHot(20, 4))).toBeCloseTo(0, 10);
    expect(scoreChoiceForecast("one_of", wrong, oneHot(20, 4))).toBeCloseTo(1, 10);
  });
  it("one_of: two options reduces to the binary Brier", () => {
    const probs = [{ optionId: 10, probability: 0.7 }, { optionId: 20, probability: 0.3 }];
    expect(scoreChoiceForecast("one_of", probs, oneHot(10, 2))).toBeCloseTo(scoreBinaryForecast(0.7, true), 10);
  });
  it("any_of: 50% on every option scores 0.25 regardless of count", () => {
    ...
    expect(scoreChoiceForecast("any_of", half(2), oneHot(10, 2))).toBeCloseTo(0.25, 10);
    expect(scoreChoiceForecast("any_of", half(4), oneHot(10, 4))).toBeCloseTo(0.25, 10);
  });
  it("any_of: is the mean of per-option binary Briers", () => {
    ...
    expect(scoreChoiceForecast("any_of", probs, outs)).toBeCloseTo((0.01 + 0.64) / 2, 10);
  });
  it("matches options by id, not by array order", () => {
    const probs = [{ optionId: 20, probability: 1 }, { optionId: 10, probability: 0 }];
    expect(scoreChoiceForecast("one_of", probs, oneHot(20, 2))).toBeCloseTo(0, 10);
  });
});
