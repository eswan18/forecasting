...
describe('Server Action Helpers', () => {
  ...
  describe('handleServerActionResult', () => {
    ...
    it('should redirect to custom path on unauthorized error when specified', () => {
      ...
      expect(() => handleServerActionResult(result, { unauthorizedRedirect: '/custom-login' })).toThrow('NEXT_REDIRECT');
    ...
    it('should throw error on non-unauthorized errors by default', () => {
      ...
      expect(() => handleServerActionResult(result)).toThrow('Database error occurred');
      expect(redirect).not.toHaveBeenCalled();
    });
  ...
  describe('handleServerActionResultWithFallback', () => {
    ...
    it('should redirect on unauthorized error', () => {
      ...
      expect(() => handleServerActionResultWithFallback(result, [])).toThrow('NEXT_REDIRECT');
      expect(redirect).toHaveBeenCalledWith('/login');
    });
    ...
    it('should return fallback value on non-unauthorized error', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Server action error:', 'Failed to fetch data');
      expect(redirect).not.toHaveBeenCalled();
    });
    ...
    it('should use custom unauthorized redirect path', () => {
      ...
      expect(() => handleServerActionResultWithFallback(result, null, { 
    ...
    it('should handle complex fallback objects', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Server action error:', 'User not found');
    });
  });
});