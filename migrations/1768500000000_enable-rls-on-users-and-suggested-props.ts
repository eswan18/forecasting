 ...
 * - Only admins can read/write (it's an admin-only feature)
 *
...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  // Only admins can access suggested_props (it's an admin-only feature)
  await sql<void>`
    CREATE POLICY "admin_only" ON suggested_props
    USING (is_current_user_admin())
    WITH CHECK (is_current_user_admin());
  `.execute(db);
}
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql<void>`DROP POLICY "admin_only" ON suggested_props`.execute(db);
