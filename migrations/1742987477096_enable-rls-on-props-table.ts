...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql<void>`CREATE VIEW v_props WITH (security_barrier, security_invoker) AS
	...
	`.execute(db);
  await sql<void>`ALTER VIEW v_forecasts SET (security_barrier = true, security_invoker = true)`.execute(
    db,
  );
}

export async function down(db: Kysely<any>): Promise<void> {
  await sql<void>`ALTER VIEW v_forecasts RESET (security_barrier, security_invoker)`.execute(
    db,
  );
  await db.schema.dropView("v_props").execute();
